...
- **AI**: LLM abstraction layer (server/llm.ts) with dual provider support — Anthropic Claude Sonnet (via Replit AI Integrations, `AI_INTEGRATIONS_ANTHROPIC_API_KEY`) is the PRIMARY provider for ALL text/image/PDF/CSV extraction, reconciliation, duplicate check, and audit. OpenAI (via Replit AI Integrations, `AI_INTEGRATIONS_OPENAI_API_KEY`) is used ONLY for audio transcription (gpt-4o-mini-transcribe STT). `getComplexParsingProvider()` auto-selects Claude when available. All reconciliation.ts functions use llm.ts provider abstraction with withRetry(). **OCR preprocessing** (Tesseract.js, server/ocr.ts): images are pre-processed with OCR before sending to AI — if OCR confidence ≥60% and text ≥30 chars, sends extracted text instead of image (cheaper + more accurate); otherwise falls back to vision AI. **Scanned PDF OCR** (server/pdf-ocr.ts): when a PDF has no extractable text (scanned/image-based), pages are converted to images via `pdftoppm` (poppler_utils) and OCR'd with Tesseract — this enables extracting data from scanned PDFs that previously would fail. Applies to all PDF processing: reconciliation upload, template extraction, and historical import.
...
server/
  ...
  reconciliation.ts  - PDF/image/CSV extraction (pdf-parse + OpenAI) + template-aware extraction + AI-powered reconciliation engine (matches on 5 fields: patient name, procedure date, birth date, procedure, insurance) + proportional Levenshtein name matching (threshold scales with name length: ≤5 chars→1, ≤10→2, longer→20% of length, min 3) + matchConfidence score (0-100%) stored per entry + sanitizeEntry with payment method detection (PX/PIX/DINHEIRO/CARTÃO → Particular)
...
5. ConfirmEntry: User reviews/edits extracted data + value -> POST /api/entries (with _originalData for AI methods, _imageHash for duplicate tracking) -> saved to DB + corrections stored in ai_corrections -> back to Dashboard
   ...
   - Duplicate detection (3 layers): (1) Photo capture computes SHA-256 hash — exact image duplicates warned at capture time. (2) Data duplicates checked with fuzzy/normalized patient name matching on same date+description (409 response). (3) **AI-powered duplicate check** via `aiDuplicateCheck()` in llm.ts — after exact match passes, finds similar entries from same day (or ±3 days for same patient) and sends to Claude for semantic analysis. AI understands that same patient can have different procedures (not duplicate), and catches variations in writing (e.g. "Maria Silva" vs "Maria da Silva", "Consulta" vs "Consulta médica"). Only blocks with HIGH confidence. Falls back to allowing entry if AI fails. Frontend shows AI reason with purple badge when ai_detected type.
   - **AI Anomaly Scan** (full database): `aiAnomalyScan()` in llm.ts + `runAIAnomalyScan()` in audit.ts. Scans all entries in batches of 80 (with 10-entry overlap for cross-batch detection). Detects: duplicates, value outliers, missing data, suspicious patterns. Validates LLM output schema. Cooldown: 6h per user (only set after successful scan). Integrated into scheduled audit (runs during 13h/22h BRT cycles). Manual trigger: `POST /api/ai/anomaly-scan`. Creates notifications: `ai_anomaly_high` (critical) and `ai_anomaly_info` (informational). Dashboard has Brain icon button in notification panel header.
