    ...
    fetch("/api/entries/photo", {
      ...
      .then((data) => {
        if (data.success && data.extractedData) {
          setPatientName(data.extractedData.patientName);
          setProcedureDate(data.extractedData.procedureDate);
          setInsuranceProvider(data.extractedData.insuranceProvider);
          setDescription(data.extractedData.description);
        }
      })
      .catch(() => {
    ...
    <div className="min-h-screen bg-[#f6f5f8] text-slate-900 relative">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="pt-2 pb-8 text-white">
          ...
          <h2 className="text-2xl font-extrabold">Confirmar Lançamento</h2>
          <p className="text-white/80 mt-1 text-sm">Revise os dados extraídos e confirme o registro</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-8 shadow-2xl mb-8">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <p className="text-slate-500 font-medium">Processando imagem com IA...</p>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex items-center gap-3 mb-2 pb-4 border-b border-slate-100">
                ...
                <div>
                  <p className="font-bold text-slate-800">Dados extraídos com sucesso</p>
                  <p className="text-xs text-slate-400">Revise e edite os campos antes de salvar</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label className="font-semibold text-slate-700 flex items-center gap-2">
                  <Building2 className="w-4 h-4 text-[#8855f6]" />
                  Convênio
                </Label>
                <Input
              ...
              <div className="space-y-2">
                <Label className="font-semibold text-slate-700 flex items-center gap-2">
                  <FileText className="w-4 h-4 text-[#8855f6]" />
                  Procedimento
                </Label>
                <Input
                  value={description}
